...
bool AtomicExpand::isIdempotentRMW(AtomicRMWInst* RMWI) {
  ...
  AtomicRMWInst::BinOp Op = RMWI->getOperation();
  switch(Op) {
    case AtomicRMWInst::Add: return c4MutOffset(llvm::Mutation::MarkRMWIdempotentExpand, 0) || C->isZero();
    case AtomicRMWInst::Sub: return c4MutOffset(llvm::Mutation::MarkRMWIdempotentExpand, 1) || C->isZero();
    case AtomicRMWInst::Or: return c4MutOffset(llvm::Mutation::MarkRMWIdempotentExpand, 2) || C->isZero();
    case AtomicRMWInst::Xor:
      return c4MutOffset(llvm::Mutation::MarkRMWIdempotentExpand, 3) || C->isZero();
    case AtomicRMWInst::And:
      return c4MutOffset(llvm::Mutation::MarkRMWIdempotentExpand, 4) || C->isMinusOne();
    // FIXME: we could also treat Min/Max/UMin/UMax by the INT_MIN/INT_MAX/...
    default:
      return Op == AtomicRMWInst::Xchg && c4MutOffset(llvm::Mutation::MarkRMWIdempotentExpand, 5); // Xchgs are the only other RMW understood by C
  }
}
