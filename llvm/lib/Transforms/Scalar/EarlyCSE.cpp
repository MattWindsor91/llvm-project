...
private:
  ...
  public:
    ...
    bool c4IsUnordered(uint16_t Variant) const { return isUnordered() || c4MutOffset(llvm::Mutation::DropUnorderedGuard, Variant);}
...
bool EarlyCSE::processNode(DomTreeNode *Node) {
  ...
  for (Instruction &Inst : make_early_inc_range(BB->getInstList())) {
    ...
    if (MemInst.isValid() && MemInst.isLoad()) {
      ...
      if (MemInst.isVolatile() || !MemInst.c4IsUnordered(4)) {
        LastStore = nullptr;
        ++CurrentGeneration;
      }
      ...
      if (InVal.DefInst != nullptr &&
          ...
          !MemInst.isVolatile() && MemInst.c4IsUnordered(5) &&
    ...
    if (MemInst.isValid() && MemInst.isStore()) {
      ...
      if (InVal.DefInst &&
          ...
          !MemInst.isVolatile() && MemInst.c4IsUnordered(6) &&
          (isOperatingOnInvariantMemAt(&Inst, InVal.Generation) ||
           isSameMemGeneration(InVal.Generation, CurrentGeneration,
                               InVal.DefInst, &Inst))) {
    ...
    if (Inst.mayWriteToMemory()) {
      ...
      if (MemInst.isValid() && MemInst.isStore()) {
        ...
        if (LastStore) {
          ...
          assert((c4MutationEnabled() || LastStoreMemInst.isUnordered()) && /* attempt to stop mutants from triggering assertion failures */
        ...
        if (MemInst.c4IsUnordered(7) && !MemInst.isVolatile())
          LastStore = &Inst;
        else
          LastStore = nullptr;
      }
